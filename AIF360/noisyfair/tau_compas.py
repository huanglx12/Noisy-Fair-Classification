...
def performance(dataset, protected_name, C, zvrg_thresh, gyf_thresh, undenoised_thresh, denoised_thresh, lam, eta0, eta1):
...
def arrays_write_to_excel(zvrg_thresh, zvrg_acc, zvrg_sr, gyf_thresh, gyf_acc, gyf_sr, 
    ...
    excel_dir = 'Compas_' + str(protected_name) + '_' + str(eta0) + '_' + str(eta1) + '_' + str(times) + '.xlsx'
    with pd.ExcelWriter(excel_dir, engine='xlsxwriter') as writer:
        ...
        for i in range(len(undenoised_thresh)):
            l.append([undenoised_thresh[i], undenoised_acc_avg[i], undenoised_acc_std[i], undenoised_sr_avg[i], undenoised_sr_std[i]])
        ...
        for i in range(len(denoised_thresh)):
            l.append([denoised_thresh[i], denoised_acc_avg[i], denoised_acc_std[i], denoised_sr_avg[i], denoised_sr_std[i]])
...
if __name__ == '__main__':
    ...
    for i in range(times):
        ...
        print("#####################################################")
        sys.stdout.flush()
        zvrg_acc[i], zvrg_sr[i], gyf_acc[i], gyf_sr[i], undenoised_acc[i], undenoised_sr[i], denoised_acc[i], denoised_sr[i] = \
        performance(dataset, protected_name, C, zvrg_thresh, gyf_thresh, undenoised_thresh, denoised_thresh, lam, eta0, eta1)
